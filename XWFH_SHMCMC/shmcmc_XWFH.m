...
%% Load image data
FF = imread('FF_p1m0108_E11p9_T9p9_vrf_m0p3.tif');  % flat field
mask = imread('pilatus1mf_mask_20110110.tif');      % mask
...
% nssp = 3;   % use two subspaces
% sspFlag = false(nssp,length(dPotE_init));
% sspFlag(1,[1,2,3,7]) = true;          % subspace for Inorm, vz0, d_ptba, d_au
% sspFlag(2,[4,5,6,8]) = true;           % subspace for sigma_dtba, sigma_ps,d_ps,elastic_ratio
% sspFlag(3,end-15:end) = true;       % subspace for coefficient and variance 
% epsilon = [0.01,0.1];   % small epsilon for large differentials; and large epsilon for small differentials
    ...
    for issp = 1:nssp
        ...
        while done_hmcmc == 0  % to prevent ill random pp values
            try
                ...
                elseif nutsFlag == 1 && dualAvgFlag == 1
