...
lb_layeredp = [ ...
    0*ones(nlayer,1)      0*ones(nlayer,1)          0*ones(nlayer,1)      [-eps;0;0;0;-Inf;-Inf]
    ];     % lower boundary 
...
if any(x<=lb | x>=ub)
    error('start value cannot be at or outside of the bounds')
end
lbub = [lb,ub];
...
subspaceFlag = true(1,length(x1)+nsets);
XFix = [];

% --- define potention  and differential
% U = @(X)potentialE(X,nsets,qset,Iset,Iweight_set,subspaceFlag,XFix);
...
dU = @(X)admDiffFD(@potentialE,1,X,nsets,qset,Iset,Iweight_set,subspaceFlag,XFix,opts);     % Numerical finite difference method (not an auto diff method)

% --- define mass matrix
M = eye(nnz(subspaceFlag));  % default
if nutsFlag == 1 % when NUTS is choosen, force M to be unit
    M = eye(nnz(subspaceFlag)); 
end
...
if nutsFlag == 1  
    ...
    lambda_target = [10*epsilon1,50*epsilon2];    % target total number simulation length
end
...
% --- create subspaces
%subspaceFlag1 =  abs(dPotE_init)<50;
subspaceFlag1 =  true(1,length(xp));
subspaceFlag1([4,7,9,11,18]) = false;
subspaceFlag2 = ~subspaceFlag1;


% --- start MCMC
nmcmc = 3000;   % # of iterations
param_mcmc = nan(nmcmc,length(xp));    
epsilon_mcmc = nan(nmcmc,2);
dPotE_mcmc = nan(nmcmc,length(xp));

% --- the 0th iteration to store the intial values
param_mcmc0 = xp_init;
epsilon_mcmc0 = [epsilon1,epsilon2];
...
for ii=1:nmcmc
    ...
    % --- assign xp
    xp1 = xp(subspaceFlag1);
    xp2 = xp(subspaceFlag2);
    
    % --- Subspace 1: move one step in HMCMC
    % - redefine U,K,dU,dK
    U = @(X)potentialE(X,nsets,qset,Iset,Iweight_set,subspaceFlag1,xp2);
    dU = @(X)admDiffFD(@potentialE,1,X,nsets,qset,Iset,Iweight_set,subspaceFlag1,xp2,opts);     % Numerical finite difference method (not an auto diff method)
    M = eye(nnz(subspaceFlag1));  % default
    K = @(p)p*inv(M)*p'/2 + log(det(M));    % Euclidean-Gaussian kinetic energy
    dK = @(p)p*inv(M);          % differation of kinetic energy w.r.t momentum    
    ...
    while done_hmcmc == 0  % to prevent ill random pp values
        try
            if nutsFlag == 0    % use simple HMCMC
                [xp1,logp_xp1,dPotE1,logr1] = hmcmcwalk(xp1,logp_xp1,dPotE1,U,K,dU,dK,M,epsilon1,lambda_target(1));                
            elseif nutsFlag == 1 && dualAvgFlag == 1    % use NUTS with dual averaging for epsilon
                [xp1,logp_xp1,dPotE1,alpha1,nalpha1] = nuts(xp1,logp_xp1,dPotE1,U,K,dU,dK,M,epsilon1,delta_max);
            elseif nutsFlag == 1 && dualAvgFlag == 0    % use NUTS without dual averaging
                [xp1,logp_xp1,dPotE1] = nuts_noDA(xp1,logp_xp1,dPotE1,U,K,dU,dK,M,epsilon1,delta_max);
            end
            done_hmcmc = 1;
            fprintf('      %d of %d subspaces (%.1f sec) with epsilon=%.5f: ',1,2,toc,epsilon1);
%            fprintf(['[',repmat('%f ',1,6),'\b]\n'],xp([1:5,end]));     % only display 6 parameters
            fprintf(['[',repmat('%f ',1,length(xp1)),'\b]\n'],xp1);    
        catch
    ...
    % --- Subspace 2: move one step in HMCMC
    % - redefine U,K,dU,dK
    U = @(X)potentialE(X,nsets,qset,Iset,Iweight_set,subspaceFlag2,xp1);
    dU = @(X)admDiffFD(@potentialE,1,X,nsets,qset,Iset,Iweight_set,subspaceFlag2,xp1,opts);     % Numerical finite difference method (not an auto diff method)
    M = eye(nnz(subspaceFlag2));  % default
    K = @(p)p*inv(M)*p'/2 + log(det(M));    % Euclidean-Gaussian kinetic energy
    dK = @(p)p*inv(M);          % differation of kinetic energy w.r.t momentum    
    ...
    while done_hmcmc == 0  % to prevent ill random pp values
        try
            if nutsFlag == 0    % use simple HMCMC
                [xp2,logp_xp2,dPotE2,logr2] = hmcmcwalk(xp2,logp_xp2,dPotE2,U,K,dU,dK,M,epsilon2,lambda_target(2));                
            elseif nutsFlag == 1 && dualAvgFlag == 1    % use NUTS with dual averaging for epsilon
                [xp2,logp_xp2,dPotE2,alpha2,nalpha2] = nuts(xp2,logp_xp2,dPotE2,U,K,dU,dK,M,epsilon2,delta_max);
            elseif nutsFlag == 1 && dualAvgFlag == 0    % use NUTS without dual averaging
                [xp2,logp_xp2,dPotE2] = nuts_noDA(xp2,logp_xp2,dPotE2,U,K,dU,dK,M,epsilon2,delta_max);
            end
            done_hmcmc = 1;
            fprintf('      %d of %d subspaces (%.1f sec) with epsilon=%.5f: ',2,2,toc,epsilon2);
%            fprintf(['[',repmat('%f ',1,6),'\b]\n'],xp([1:5,end]));     % only display 6 parameters
            fprintf(['[',repmat('%f ',1,length(xp2)),'\b]\n'],xp2);    
        catch
            trial_hmcmc = trial_hmcmc + 1;                        
            fprintf('For the %dth iteration: re-try %d time(s)\n', ii,trial_hmcmc);
        end
    ...
    % --- collect result
    xp = nan(1,length(subspaceFlag1));
    xp(subspaceFlag1) = xp1;
    xp(subspaceFlag2) = xp2;
    
    dPotE = nan(1,length(subspaceFlag1));
    dPotE(subspaceFlag1) = dPotE1;
    dPotE(subspaceFlag2) = dPotE2;    
    param_mcmc(ii,:) = xp;
    epsilon_mcmc(ii,:) = [epsilon1,epsilon2];
    dPotE_mcmc(ii,:) = dPotE;
    ...
    if updatePlotFlag == 1
        ...
        if ii==1
           ...
           %  hline1 = [];
            ...
            set(hline4(1),'XData',1:nmcmc,'YData',epsilon_mcmc(:,1));             
            set(hline4(2),'XData',1:nmcmc,'YData',epsilon_mcmc(:,2));             
